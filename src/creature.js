export function creatureSvg(result, className = "creature") {
  ...
  const hue = Number.isFinite(species.hue) ? species.hue : 76;
  const posture = phenotype.posture === "forward" ? "is-forward" : "is-resting";
  ...
  return `<svg class="${className} ${posture}" style="--creature-hue:${hue}" viewBox="0 0 250 240" role="img" aria-label="${result.id} の姿">
    <g class="creature-pose">
