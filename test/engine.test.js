...
import { scoresFor, makeResult, resultShareText } from "../src/engine.js";
...
test("share text contains the individual encounter without exposing scores", () => {
  const result = makeResult({ seed: 1, answers: [0, 0, 0, 0, 2, 0], artifact: "穴のあいた石" }, scenes, new Date("2026-01-01"));
  const text = resultShareText(result, "しゃがんで待つ");
  assert.match(text, new RegExp(result.id));
  assert.match(text, /持ちもの: 穴のあいた石/);
  assert.match(text, /最初に「しゃがんで待つ」を選んだ。/);
  assert.doesNotMatch(text, /approach|tempo|social|structure|retention/);
});
