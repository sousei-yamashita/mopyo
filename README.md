...
- `gh-pages` への書き込みはPreview更新・cleanup・`main`公開をすべて同じ workflow concurrency group で直列化し、進行中jobの取消しもしません。各writerは毎回「同一repositoryで現在openなPR一覧」を見て preview 群全体を再構築するため、pending の cleanup / publish が後続runに置換されても最新runで状態が収束します。
